    ...
    for phase in range(phases):
        for round_nr in range(rounds_per_phase):
            ...
            for node in graph.nodes():
                ...
                computation = f(
                    phase, round_nr, node_values[node], neighbor_values, D
                )
            ...
            plot_graph(
                G,
                node_values,
                f"mis-{phase * rounds_per_phase + round_nr:03d}.png",
            )
    return node_values


def graph_step(
    phase: int, round_nr: int, own: set, in_values: list[set], D: int
) -> set:
    """
...
def plot_graph(
    G: nx.Graph, node_values: dict[NodeView, set], fname: str
) -> None:
    node_colors = []
